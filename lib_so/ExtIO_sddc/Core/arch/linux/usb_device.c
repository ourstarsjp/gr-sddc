...
static int usb_interface_count = 0;
                              ...
                              uint32_t size)
{
  ...
  if (needs_firmware) {
    int load_count = 0;
    int loop_count = 0;
    int device_num = usb_device_count_devices();
    ...
    usleep(500 * 1000L);
    // Setup the firmware to all devices.
    while(load_count < device_num) {
      for(int i = 0; i < device_num; i++) {
        needs_firmware = 0;
        dev_handle = find_usb_device(i, ctx, &device, &needs_firmware);
        if (dev_handle == 0) {
          goto FAIL1;
        }
        if (needs_firmware) {
      ...
      if (loop_count > device_num+100) {
        log_error("load_image() too long loop.", __func__, __FILE__, __LINE__);
        goto FAIL1;
      }
      loop_count++;
    }
