...
bool fx3handler::Open(int index, uint8_t* fw_data, uint32_t fw_size)
{
    dev_index = index;
//    usb_device_init((const char*)fw_data, fw_size);
    dev = usb_device_open(dev_index, (const char*)fw_data, fw_size);
    return dev != nullptr;
}

bool fx3handler::Control(FX3Command command, uint8_t data)
{
    return usb_device_control(this->dev, command, 0, dev_index, (uint8_t *) &data, sizeof(data), 0) == 0;
}
...
{
    return usb_device_control(this->dev, TESTFX3, 0, dev_index, (uint8_t *) data, sizeof(*data), 1) == 0;
}
